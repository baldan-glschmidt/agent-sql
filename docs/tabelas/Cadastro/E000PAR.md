# E000PAR

## Descrição

Tabelas - Recebimento de Documentos Eletrônicos - Notas Fiscais de Entrada - Parcelas

---

## Resumo

- Campos: 50
- Chave Primária: 1 campo(s)
- Índices: 3
- Relacionamentos: 0

---

## Campos

| Campo | Tipo | Nulo | Descrição |
|--------|------|------|-----------|
| CgcFil | Number(015,0) | Não | Número do cadastro nacional de pessoa jurídica da filial da empresa |
| DocIdeFil | String(014) | Sim | Número do cadastro nacional de pessoa jurídica da filial da empresa |
| CgcFor | Number(014,0) | Não | Número do CNPJ ou CPF do fornecedor |
| DocIdeFor | String(014) | Sim | Número do CNPJ ou CPF do fornecedor |
| ChvNel | String(050) | Não | Chave de acesso da nota fiscal eletrônica |
| CodPar | Number(003,0) | Não | Sequência de parcelas da nota fiscal de entrada |
| NumNfc | Number(009,0) | Sim | Número da nota fiscal de entrada |
| CodSnf | String(003) | Não | Código da série da nota fiscal de entrada |
| CodCrp | String(003) | Sim | Código do grupo a pagar |
| NumTit | String(015) | Não | Número do título a ser gerado no contas a pagar |
| CodTpt | String(003) | Sim | Código do tipo de título a ser gerado no contas a pagar |
| CodFcr | String(003) | Sim | Código da moeda ou índice como fator de correção (financeiro) |
| DatFcr | Date | Sim | Data da cotação da moeda ou índice para o fator de correção (financeiro) |
| VctPar | Date | Sim | Data de vencimento da parcela da nota fiscal de entrada |
| VlrPar | Number(015,2) | Sim | Valor da parcela da nota fiscal de entrada |
| PerDdp | Number(005,2) | Sim | Percentual de desconto da parcela da nota fiscal de entrada |
| QtdDdd | Number(003,0) | Sim | Quantidade de dias de tolerância para o desconto da parcela |
| CodPor | String(004) | Sim | Código do portador a ser lançado o título no contas a pagar |
| CodCrt | String(002) | Sim | Código da carteira a ser lançado o título no contas a pagar |
| CodNtg | Number(004,0) | Sim | Código da natureza de gasto |
| ObsPar | String(250) | Sim | Texto da observação |
| UsuGer | Number(010,0) | Sim | Usuário responsável pela geração do registro |
| DatGer | Date | Sim | Data da geração do registro |
| HorGer | Number(005,0) | Sim | Hora da geração do registro |
| VlrDsc | Number(015,2) | Sim | Valor do desconto negociado a ser concedido ao título a pagar |
| JrsNeg | Number(015,2) | Sim | Valor dos juros negociados |
| MulNeg | Number(015,2) | Sim | Valor da multa negociada |
| OutNeg | Number(015,2) | Sim | Valor de outros valores negociados |
| DatNeg | Date | Sim | Data base dos valores negociados (data até) |
| CodBan | String(003) | Sim | Número do banco na FEBRABAN |
| CodAge | String(007) | Sim | Número da agência do banco |
| CcbFor | String(014) | Sim | Número da conta corrente do fornecedor no banco |
| CodFav | Number(014,0) | Sim | Número do CNPJ ou CPF do favorecido |
| DocIdeFav | String(014) | Sim | Número do CNPJ ou CPF do favorecido |
| GenA01 | String(080) | Sim | Tratamentos externos ao ERP nas parcelas da nota - A1 |
| GenA02 | String(080) | Sim | Tratamentos externos ao ERP nas parcelas da nota - A2 |
| GenA03 | String(080) | Sim | Tratamentos externos ao ERP nas parcelas da nota - A3 |
| GenA04 | String(080) | Sim | Tratamentos externos ao ERP nas parcelas da nota - A4 |
| GenA05 | String(080) | Sim | Tratamentos externos ao ERP nas parcelas da nota - A5 |
| GenA06 | String(080) | Sim | Tratamentos externos ao ERP nas parcelas da nota - A6 |
| GenN01 | Number(013,2) | Sim | Tratamentos externos ao ERP nas parcelas da nota - N1 |
| GenN02 | Number(013,2) | Sim | Tratamentos externos ao ERP nas parcelas da nota - N2 |
| GenN03 | Number(013,2) | Sim | Tratamentos externos ao ERP nas parcelas da nota - N3 |
| GenN04 | Number(013,2) | Sim | Tratamentos externos ao ERP nas parcelas da nota - N4 |
| GenN05 | Number(013,2) | Sim | Tratamentos externos ao ERP nas parcelas da nota - N5 |
| GenN06 | Number(013,2) | Sim | Tratamentos externos ao ERP nas parcelas da nota - N6 |
| CodFpg | Number(002,0) | Sim | Código da forma de pagamento |
| IndPag | String(001) | Sim | Indicativo da forma de pagamento |
| IdeUni | String(050) | Não | Identificador único alfanumérico |
| IdeNfc | String(050) | Não | Identificador único alfanumérico |

---

## Chave Primária

- IdeUni

---

## Índices

### E000PARIndice1

**Tipo:** Não unico

Campos:
- CgcFil
- CgcFor
- ChvNel
- CodPar

### E000PARIndice2

**Tipo:** Não unico

Campos:
- DocIdeFil
- DocIdeFor
- ChvNel
- CodPar

### E000PARIndice3

**Tipo:** Não unico

Campos:
- IdeNfc

---

## Relacionamentos

Nenhum relacionamento cadastrado.
